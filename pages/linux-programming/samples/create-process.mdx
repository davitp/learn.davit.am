...
int main(){
    ...
    if(child == 0){
        ...
        std::cout << "This will only be executed in the parent process. PID: " << getpid() << std::endl;
    }

    // this message will be printed twice: from both parent and child processes
    std::cout << "General hello from " << getpid() << std::endl;

    // wait for the child only in the parent
    if(child > 0){
        
        // a variable to keep the exit code of the child process once it finishes
        int childStatus = -1;
